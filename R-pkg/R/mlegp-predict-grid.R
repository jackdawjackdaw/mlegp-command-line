...
plot.main.effects.pca <- function(fit.list, nobs=6, file.name.stub="main-effects-"){
  ...
  for(i in 1:nobs){
    if(i == 1){
      d <- plot.main.custom(fit.list[[i]])
    } else {
      d <- plot.main.custom(fit.list[[i]], plot.leg=FALSE)
    }
...
plot.main.effects.true <- function(fit.pca, plot.leg=TRUE, ylim=c(-3,3))
{
  ...
  scaled.mean.vec <- attr(model.scaled, "scaled:center")
  scaled.scale.vec <- attr(model.scaled, "scaled:scale")

  main.effects <- vector("list", nbins)
  for(i in 1:nbins) ## entries are observables 1:nbins
  ...
  for(i in 1:nparams){ ## loop over all params
    ...
    for(j in 1:nbins)  ## fill in the effect of the i'th parameter for each observable
  ...
  ## plot everything
  par(mfrow=c(2,round(nobs.max/2)))
  for(i in 1:nbins){
    ## now make a plot
    for(j in 1:nparams){
      if(j==1){
